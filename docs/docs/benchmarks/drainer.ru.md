...
!!! danger "Легаси-режим здесь не просто медленнее — он зависает"
    В легаси-режиме `close()` держит блокировку транспорта на всё время
    ожидания ответа `CloseOk` от брокера. Если в этот самый момент у
    фонового консьюмера в разгаре собственное блокирующее чтение, он ждёт
    ту же самую блокировку, чтобы это чтение выполнить, — получается
    circular wait (взаимное ожидание по кругу, классический дедлок), что
    подтверждено дампами стека потоков прямо в этом бенчмарке. Это давнее
    свойство легаси-пути: раньше оно было известно как теоретическая
    возможность, а этот бенчмарк впервые измерил его на практике.
    `dedicated_drainer` избегает этого структурно: `close()` никогда не
    держит блокировку транспорта во время такого ожидания, потому что
    само чтение из сокета выполняет поток дренера, а не вызывающий поток.

Бенчмарк проверяет для `dedicated_drainer` нулевое число дедлоков и P95
меньше 4 секунд; число дедлоков в легаси-режиме просто фиксируется как
ожидаемый результат этого бенчмарка, а не проверяется на равенство нулю.
