...
### 5. Dedicated Drainer (выделенный дренер)

Сравнивает опциональный режим `dedicated_drainer` с легаси-путём чтения:
задержка публикации при зависшем консьюмере, паритет по пропускной
способности, скорость восстановления после сброса соединения и
ограниченная по времени остановка под нагрузкой.

**Что измеряем:**

- Задержку публикации, пока другой поток блокируется в `drain_events()`
- Паритет пропускной способности и задержки при одинаковом числе потоков
- Время распространения ошибки на все потоки после сброса TCP-соединения
- Исход `close()` (дедлок или завершение за ограниченное время) под нагрузкой

[Подробнее →](drainer.md)
